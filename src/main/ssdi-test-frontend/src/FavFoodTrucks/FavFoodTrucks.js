...
function FavFoodTrucks() {
    ...
    useEffect(() => {
        if (currentLocation && distance) {
            ...
            axios.get(url + '/getFavFoodTrucksByUserId?id=' + Number(localStorage.getItem("FoodTruckAppID")))
                .then((response) => {
                    const updatedFoodTrucks = response.data.map((foodTruck) => {
                        return {
                            ...foodTruck,
                            isFav: true,
                        };
    ...
    const toggleFav = (id, event) => {
        ...
        axios
            .post(url + "/addOrRemoveFavFoodTruck", {
            ...
            .then((response) => {
                if (response.data.message === "Food Truck added to favorites successfully") {
                    ...
                    handleToast("Food Truck removed to favorites successfully");
                    console.log(response.data.message);
                    const index = foodTrucks.findIndex((foodTruck) => foodTruck.id === id);
                    if (index !== -1) {
                        const newFoodTrucks = [...foodTrucks];
                        newFoodTrucks.splice(index, 1);
